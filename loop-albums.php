    ...
    if ( page <= parseInt(total_page) ) {
      ...
      $.post( _BASE_URL + 'public/gallery_photos/get_albums', data, function( response ) {
        ...
        for (var z in rows) {
          const row = rows[ z ];
          const template = `
          <figure class="shadow-md bg-white overflow-hidden album">
            <img src="${_BASE_URL}media_library/albums/${row.album_cover}" alt="${row.alubm_title}" class="h-48 w-full object-fit object-cover flex items-center justify-center bg-gray-300">
